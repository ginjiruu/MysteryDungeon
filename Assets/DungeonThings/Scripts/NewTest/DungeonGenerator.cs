...
public class DungeonGenerator : MonoBehaviour{
	...
	*/
	...
	public	static	DungeonTile[,]	_dungeon;	//2D-Array that stores the current type of a tile
	public	static	int[,]	_regions;   //2D-Array
	#region disjointSet
	public TileStructure[] allTiles;
	public DisjointSet tileArray;
	...
	//Determining if the tile is part of a room set
	//public bool IsRoomTile(Int2 _location)
	//{
	//	if (allTiles[((_location.y - 1) * _dungeonWidth) + _location.x].isPartOfRoom)
	//		return true;
	//	else if (tileArray.Parent[((_location.y - 1) * _dungeonWidth) + _location.x] != tileArray.Find(_location.x + _location.y))
	//		return true;
	//	else
	//		return false;
	//}
	...
	public void SetTile(IntVector2 pos, DungeonTile tile)	{	_dungeon[pos.x,pos.y] = tile;			}   //OCCASIONAL ERROR
	public DungeonTile GetTile(IntVector2 pos) { return _dungeon[pos.x, pos.y]; }
	...
	private	IntVector2 FindFloorInDungeon(Dungeon dungeon){
		...
		for(int x = 0; x < xSize; x++){
			for(int y = 0; y < ySize; y++){
				if(dungeon.tiles[x,y] == DungeonTile.Floor){
	...
	private void GenerateByTilemap(DungeonTile[,] grid)
	{
		int xSize = grid.GetLength(0);
		int ySize = grid.GetLength(1);
		for (int x = 0; x < xSize; x++)
			...
			for (int y = 0; y < ySize; y++)
			{
				switch (grid[x, y])
				{
					...
					case DungeonTile.Floor:
						...
						tilemap.SetTile(new Vector3Int(x, y, 0), floors);
						break;
					case DungeonTile.RoomFloor: tilemap.SetTile(new Vector3Int(x, y, 0), roomFloor); break;
					case DungeonTile.Wall: tilemap.SetTile(new Vector3Int(x, y, 0), walls); break;
					case DungeonTile.Door: tilemap.SetTile(new Vector3Int(x, y, 0), floors); break;
				}
			}
		}
...
public	Dungeon GenerateRoomTemplateDungeon(	DungeonParameters dParams = null, string previousDungeon = "" ){
	...
	//BEGIN TRIAL
		...
		for(int x = 1; x < _dungeonWidth-1; x++){
			for(int y = 1; y < _dungeonHeight-1; y++){	
				switch(_dungeon[x,y]){
					case DungeonTile.None:	_regions[x,y] = -1000;	break;
					case DungeonTile.Wall:	_regions[x,y] = -100;	break;
					case DungeonTile.Door:	_regions[x,y] = -10;	break;
				}							
			}
		}
	...
	private	bool RoomPlacementTrial( bool firstIsEntrance, PortalTemplate usedEntrance, int minimumRoomCount) {	
		...
		_dungeon	= new DungeonTile[_dungeonWidth, _dungeonHeight];
		...
		foreach( Door door in compatibleDoors){
			...
			foreach( IntVector2 point in doorPoints){
				...
				//	print (curPos.x+","+curPos.y);
					...
					if( rDungeon.tiles[curPos.x,curPos.y] != DungeonTile.None)	{ reachedBorder = false; break; }	//we hit an obstacle
			...
			if(reachedBorder){
				...
				foreach( IntVector2 point in doorPoints)		{
					...
					if(IntPointInBounds( pointD, xSize, ySize)){
						rDungeon.tiles[pointD.x, pointD.y]	= DungeonTile.Door;
					}else{
				...
				foreach( IntVector2 bridgeTile in bridgeTiles)	{
					rDungeon.tiles[bridgeTile.x, bridgeTile.y]					= DungeonTile.Floor;
				}
				...
				foreach( IntVector2 bridgeTile in bridgeTiles)	{
					foreach(IntVector2 v2 in Directions8){
						...
						if( IntPointInBounds( point, xSize, ySize) && rDungeon.tiles[point.x, point.y] == DungeonTile.None ){
							rDungeon.tiles[point.x, point.y] = DungeonTile.Wall;
						}
					}
				}		
	...
	private	void FillVoids(	RoomDungeon rDungeon ){
		...
		for(int x = 0; x < xSize; x++){
			for(int y = 0; y < ySize; y++){
				if(rDungeon.tiles[x,y] == DungeonTile.None){
					switch( dir ){
						case PortalDirection.North:	if( y < ySize/2 )	{ rDungeon.tiles[x,y] = DungeonTile.Wall;	}	break;
						case PortalDirection.East:	if( x > xSize/2 )	{ rDungeon.tiles[x,y] = DungeonTile.Wall;	}	break;
						case PortalDirection.South:	if( y > ySize/2 )	{ rDungeon.tiles[x,y] = DungeonTile.Wall;	}	break;
						case PortalDirection.West:	if( x < xSize/2 )	{ rDungeon.tiles[x,y] = DungeonTile.Wall;	}	break;						
					}					
				}
			}
		}
	...
	//			
				...
				for(int x = 0; x < xSizeRoom; x++){
					...
					//	collision = (_dungeon[x +offX,y +offY] == Tile.Floor) && roomTemplate.tiles[x,y] != Tile.None;
						...
						collision = !( (_dungeon[x +offX,y +offY] == DungeonTile.None) || roomTemplate.tiles[x,y] == DungeonTile.None ) || (_dungeon[x +offX,y +offY] == DungeonTile.Wall && roomTemplate.tiles[x,y] == DungeonTile.Wall);
				...
				if(!collision){	//create actual room
					...
					newRoom.tiles	[plugDoor.x, plugDoor.y] = DungeonTile.Door;
					placedRoom.tiles[jackDoor.x, jackDoor.y] = DungeonTile.Door;
					
					_dungeon[placedRoom.x+jackDoor.x, placedRoom.y+jackDoor.y] = DungeonTile.Door;	//retroactively carve door into dungeon, placedRoom has been carved before
										
					//2x2 Door
					if(jackDoor.doorLength > 1){
						newRoom.tiles		[plugDoor.x	+(verticalDoor?1:0), plugDoor.y	+(verticalDoor?0:1)] = DungeonTile.Door;
						placedRoom.tiles[jackDoor.x	+(verticalDoor?1:0), jackDoor.y	+(verticalDoor?0:1)] = DungeonTile.Door;
						_dungeon[placedRoom.x+jackDoor.x+(verticalDoor?1:0), placedRoom.y+jackDoor.y+(verticalDoor?0:1)] = DungeonTile.Door;												
					}
	...
	private	void CarveRoomIntoDungeon( Room room, int offX, int offY ){
		...
		for(int x = 0; x < xSizeRoom; x++){
			for(int y = 0; y < ySizeRoom; y++){						
				if(room.tiles[x,y] != DungeonTile.None){	//copy everything except Tile.None into dungeon							
					_dungeon[x +offX,y +offY] = room.tiles[x,y];							
					if(room.tiles[x,y] == DungeonTile.Floor){	_regions[x +offX,y +offY] = currentRoom;	}	//set region too					
				}
			}
		}				
	...
	private	Room CreateRoomFromTemplate(RoomTemplate roomTemplate){
		...
		DungeonTile[,] roomTemplateTilesCopy = new DungeonTile[xSizeRoom, ySizeRoom];		
	...
	private	IntVector2 FindFloorInRoom(Room room, int min){
		...
		for(int x = min; x < xSize; x++){
			for(int y = min; y < ySize; y++){
				if(room.tiles[x,y] == DungeonTile.Floor){
	...
	private	DungeonTile[,] CreateBorderedRectRoom(int xSize, int ySize){
		
		DungeonTile[,]	tiles = new DungeonTile[xSize,ySize];
	...
	private	DungeonTile[,] CreateUnborderedRectRoom(int xSize, int ySize){		
		DungeonTile[,]	tiles = new DungeonTile[xSize,ySize];						
		//insides
		for(int x = 0; x < xSize; x++){
			for(int y = 0; y < ySize; y++){			
	...
	private	void MirrorTiles( DungeonTile[,] roomTiles ){
		...
		for(int x = 0; x < xSize; x++){
			if(x < xSize/2){
				for(int y = 0; y < ySize; y++){	//swap
					DungeonTile temp = roomTiles[x,y];
	...
	private	void TransposeTiles( DungeonTile[,] roomTiles ){
		
	}
	
	private	void RotateTiles( DungeonTile[,] roomTiles ){
		
	}
	
	private	void IngrowRoomBorderTiles( DungeonTile[,] roomTiles ){	//D array is passed by ref
	...
	private	DungeonTile[,] OverlayNewRoom(DungeonTile[,] oldTiles){
		...
		//ovelay room tiles
		DungeonTile[,] overlay = CreateUnborderedRectRoom(xSizeOverlay, ySizeOverlay);
		...
		DungeonTile[,] newTiles = new DungeonTile[newXSize, newYSize];
			...
			for(int y = 0; y < newYSize; y++){
				...
				if(x >= offX && y >= offY){									// if on or past the origin of overlay
					if(x < offX +xSizeOverlay && y < offY +ySizeOverlay){	// if room is completely contained we have to check if the overlay array isnt out of bounds
						DungeonTile oldTile	= newTiles[x,y];
						DungeonTile overTile	= overlay[x -offX, y -offY];
						
						if			( oldTile == DungeonTile.None ){				//if new is overlaying None: replace
							newTiles[x,y] = overTile;
						}else if	( oldTile == DungeonTile.Floor || overTile == DungeonTile.Floor){	//floor always wins, overrwriting old walls too if walls already exist on starting room
							newTiles[x,y] = DungeonTile.Floor;
						}
					}
				}
	...
	private	int adjacentWalls( DungeonTile[,] tiles, int x, int y){
		int adj = 0;
		if( tiles[x-1,y+0] == DungeonTile.Wall ){adj++;}
		if( tiles[x+0,y+1] == DungeonTile.Wall ){adj++;}
		if( tiles[x+1,y+0] == DungeonTile.Wall ){adj++;}
		if( tiles[x+0,y-1] == DungeonTile.Wall ){adj++;}
		return adj;
	}
	...
	private	void FillRoomCavities( RoomTemplate roomTemplate ){
		...
		for(int x = 1; x < xSize-1; x++){
			for(int y = 1; y < ySize-1; y++){
				if(roomLabels[x,y] == 0 && roomTemplate.tiles[x,y] == DungeonTile.Floor){
					regionLabel++;
		...
		for(int x = 1; x < xSize-1; x++){
			for(int y = 1; y < ySize-1; y++){
				if(	roomLabels[x,y] != largestRegionLabel && roomLabels[x,y]!= 0 ){	roomTemplate.tiles[x,y]= DungeonTile.Wall;	}
			}
		}		
	}
	
	private int FillNeighbors(ref DungeonTile[,] tiles, int label, int x, int y, ref int[,] roomLabels, int depth){
		...
		if(roomLabels[x+0, y+1] == 0 && tiles[x+0, y+1] == DungeonTile.Floor){	hits += FillNeighbors(ref tiles, label, x+0, y+1, ref roomLabels, depth);	}
		if(roomLabels[x+1, y+0] == 0 && tiles[x+1, y+0] == DungeonTile.Floor){	hits += FillNeighbors(ref tiles, label, x+1, y+0, ref roomLabels, depth);	}
		if(roomLabels[x+0, y-1] == 0 && tiles[x+0, y-1] == DungeonTile.Floor){	hits += FillNeighbors(ref tiles, label, x+0, y-1, ref roomLabels, depth);	}
		if(roomLabels[x-1, y+0] == 0 && tiles[x-1, y+0] == DungeonTile.Floor){	hits += FillNeighbors(ref tiles, label, x-1, y+0, ref roomLabels, depth);	}
		return hits;
	}
	...
	private	void CutRoom( RoomTemplate roomTemplate ){
		...
		for(int x = 0; x < xSize; x++){
			for(int y = 0; y < ySize; y++){	
				...
				if( roomTemplate.tiles[x,y] == DungeonTile.Wall ){
					...
					obsolete	= obsolete&& ( ignoreN||				( roomTemplate.tiles[x+0,y+1] == DungeonTile.Wall || roomTemplate.tiles[x+0,y+1] == DungeonTile.None) );	//N
					obsolete	= obsolete&& ( ignoreN||	ignoreE||	( roomTemplate.tiles[x+1,y+1] == DungeonTile.Wall || roomTemplate.tiles[x+1,y+1] == DungeonTile.None) );	//NE
					obsolete	= obsolete&& ( ignoreE||				( roomTemplate.tiles[x+1,y+0] == DungeonTile.Wall || roomTemplate.tiles[x+1,y+0] == DungeonTile.None) );	//E
					obsolete	= obsolete&& ( ignoreS||	ignoreE||	( roomTemplate.tiles[x+1,y-1] == DungeonTile.Wall || roomTemplate.tiles[x+1,y-1] == DungeonTile.None) );	//SE
					obsolete	= obsolete&& ( ignoreS||				( roomTemplate.tiles[x+0,y-1] == DungeonTile.Wall || roomTemplate.tiles[x+0,y-1] == DungeonTile.None) );	//S
					obsolete	= obsolete&& ( ignoreS||	ignoreW||	( roomTemplate.tiles[x-1,y-1] == DungeonTile.Wall || roomTemplate.tiles[x-1,y-1] == DungeonTile.None) );	//SW
					obsolete	= obsolete&& ( ignoreW||				( roomTemplate.tiles[x-1,y+0] == DungeonTile.Wall || roomTemplate.tiles[x-1,y+0] == DungeonTile.None) );	//W
					obsolete	= obsolete&& ( ignoreN||	ignoreW||	( roomTemplate.tiles[x-1,y+1] == DungeonTile.Wall || roomTemplate.tiles[x-1,y+1] == DungeonTile.None) );	//NW
					
					if(obsolete){roomTemplate.tiles[x,y] = DungeonTile.None;}
	...
	private	void CreateBorders( DungeonTile[,] tiles ){
		...
		for(int x = 0; x < xSize; x++){
			for(int y = 0; y < ySize; y++){	
				...
				if( tiles[x,y] == DungeonTile.Floor ){
					...
					noneAdj	= noneAdj|| ( ignoreN||				( tiles[x+0,y+1] == DungeonTile.None) );	//N
					noneAdj	= noneAdj|| ( ignoreN||	ignoreE||	( tiles[x+1,y+1] == DungeonTile.None) );	//NE
					noneAdj	= noneAdj|| ( ignoreE||				( tiles[x+1,y+0] == DungeonTile.None) );	//E
					noneAdj	= noneAdj|| ( ignoreS||	ignoreE||	( tiles[x+1,y-1] == DungeonTile.None) );	//SE
					noneAdj	= noneAdj|| ( ignoreS||				( tiles[x+0,y-1] == DungeonTile.None) );	//S
					noneAdj	= noneAdj|| ( ignoreS||	ignoreW||	( tiles[x-1,y-1] == DungeonTile.None) );	//SW
					noneAdj	= noneAdj|| ( ignoreW||				( tiles[x-1,y+0] == DungeonTile.None) );	//W
					noneAdj	= noneAdj|| ( ignoreN||	ignoreW||	( tiles[x-1,y+1] == DungeonTile.None) );	//NW
					
					//if one adjacent block is Floor, condition is met
					flooAdj	= flooAdj|| ( !ignoreN&&				( tiles[x+0,y+1] == DungeonTile.Floor));	//N
					flooAdj	= flooAdj|| ( !ignoreN&&	!ignoreE&&	( tiles[x+1,y+1] == DungeonTile.Floor));	//NE
					flooAdj	= flooAdj|| ( !ignoreE&&				( tiles[x+1,y+0] == DungeonTile.Floor));	//E
					flooAdj	= flooAdj|| ( !ignoreS&&	!ignoreE&&	( tiles[x+1,y-1] == DungeonTile.Floor));	//SE
					flooAdj	= flooAdj|| ( !ignoreS&&				( tiles[x+0,y-1] == DungeonTile.Floor));	//S
					flooAdj	= flooAdj|| ( !ignoreS&&	!ignoreW&&	( tiles[x-1,y-1] == DungeonTile.Floor));	//SW
					flooAdj	= flooAdj|| ( !ignoreW&&				( tiles[x-1,y+0] == DungeonTile.Floor));	//W
					flooAdj	= flooAdj|| ( !ignoreN&&	!ignoreW&&	( tiles[x-1,y+1] == DungeonTile.Floor));	//NW
					
					if(noneAdj && flooAdj){tiles[x,y] = DungeonTile.Wall;}
	...
	private	void GeneratePotentialDoors(RoomTemplate roomTemplate){
		...
		DungeonTile[,] roomTemplateTilesCopy = new DungeonTile[xSize,ySize];		
		...
		for(int x = 0; x < xSize; x++){
			for(int y = 0; y < ySize; y++){
				if( roomTemplate.tiles[x,y] == DungeonTile.Wall ){
					...
					//if one adjacent block is empty, the block isn't obsolete
					DungeonTile nTile	= noneN ?	DungeonTile.None:	roomTemplate.tiles[x+0,y+1];	//N
					DungeonTile sTile	= noneS ?	DungeonTile.None:	roomTemplate.tiles[x+0,y-1];	//S
					DungeonTile eTile	= noneE ?	DungeonTile.None:	roomTemplate.tiles[x+1,y+0];	//E
					DungeonTile wTile	= noneW ?	DungeonTile.None:	roomTemplate.tiles[x-1,y+0];	//W
					
					//Doordirections, N-door means leaving room in N direction
					
					if		( nTile == DungeonTile.None  && sTile == DungeonTile.Floor && eTile == DungeonTile.Wall  && wTile == DungeonTile.Wall	){	roomTemplateTilesCopy[x,y] = DungeonTile.DoorNorth;	}	//door North
					else if	( nTile == DungeonTile.Floor && sTile == DungeonTile.None  && eTile == DungeonTile.Wall  && wTile == DungeonTile.Wall	){	roomTemplateTilesCopy[x,y] = DungeonTile.DoorSouth;	}	//door South
					else if	( nTile == DungeonTile.Wall  && sTile == DungeonTile.Wall  && eTile == DungeonTile.None  && wTile == DungeonTile.Floor	){	roomTemplateTilesCopy[x,y] = DungeonTile.DoorEast;		}	//door East
					else if	( nTile == DungeonTile.Wall  && sTile == DungeonTile.Wall  && eTile == DungeonTile.Floor && wTile == DungeonTile.None	){	roomTemplateTilesCopy[x,y] = DungeonTile.DoorWest;		}	//door West
				}
			}
		}	
		...
		for(int x = 0; x < xSize; x++){
			for(int y = 0; y < ySize; y++){
				...
				if(roomTemplateTilesCopy[x,y] != DungeonTile.None && roomTemplateTilesCopy[x,y] != DungeonTile.Wall){	//is door?
					...
					//if one adjacent block is empty, the block isn't obsolete
					DungeonTile nTile	= noneN ?	DungeonTile.None:	roomTemplateTilesCopy[x+0,y+1];	//N
					//Tile sTile	= noneS ?	Tile.None:	roomTemplateTilesCopy[x+0,y-1];	//S
					DungeonTile eTile	= noneE ?	DungeonTile.None:	roomTemplateTilesCopy[x+1,y+0];	//E
					//Tile wTile	= noneW ?	Tile.None:	roomTemplateTilesCopy[x-1,y+0];	//W
										
					//Doordirections, N-door means leaving room in N direction
					
					if			( roomTemplateTilesCopy[x,y] == DungeonTile.DoorNorth	|| roomTemplateTilesCopy[x,y] == DungeonTile.DoorSouth) {	//door North
						
						DoorDirection dir = roomTemplateTilesCopy[x,y] == DungeonTile.DoorNorth? DoorDirection.North: DoorDirection.South;
						...
						roomTemplateTilesCopy[x,y]		= DungeonTile.Wall;	//remove DoorMark
						
						
					}else if	( roomTemplateTilesCopy[x,y] == DungeonTile.DoorEast	|| roomTemplateTilesCopy[x,y] == DungeonTile.DoorWest) {	//door South
													
						DoorDirection dir = roomTemplateTilesCopy[x,y] == DungeonTile.DoorEast? DoorDirection.East: DoorDirection.West;
	...
	private	List<Door> GenerateDoorList( DungeonTile[,] tiles){
		...
		for(int x = 0; x < xSize; x++){
			for(int y = 0; y < ySize; y++){
				...
				switch(tiles[x,y]){		//	▼ only create door for lower tile, this function only uses doors of lenght 2						//AfterDoor has been added to list, set Tiles to wall, doorLink algorithm will carve them out again
					case DungeonTile.DoorNorth:	if(tiles[x+1,y]==DungeonTile.DoorNorth){potentialDoors.Add(new Door( DoorDirection.North, 2, x,y	));}	tiles[x,y]=DungeonTile.Wall;	break;
					case DungeonTile.DoorSouth:	if(tiles[x+1,y]==DungeonTile.DoorSouth){potentialDoors.Add(new Door( DoorDirection.South, 2, x,y	));}	tiles[x,y]=DungeonTile.Wall;	break;
					case DungeonTile.DoorEast:		if(tiles[x,y+1]==DungeonTile.DoorEast) {potentialDoors.Add(new Door( DoorDirection.East,  2, x,y	));}	tiles[x,y]=DungeonTile.Wall;	break;
					case DungeonTile.DoorWest:		if(tiles[x,y+1]==DungeonTile.DoorWest) {potentialDoors.Add(new Door( DoorDirection.West,  2, x,y	));}	tiles[x,y]=DungeonTile.Wall;	break;
				}
			}
		}
	...
	private	List<IntVector2> GenerateSpawnerList( DungeonTile[,] tiles){
		...
		for(int x = 0; x < xSize; x++){
			for(int y = 0; y < ySize; y++){	
				switch(tiles[x,y]){
					case DungeonTile.Spawner:	spawners.Add(new IntVector2(x,y));	tiles[x,y]=DungeonTile.Floor;	break;
				}
			}
		}
	...
	private	void RecursiveRoomExtract(ref Color32[,] pixels, HashSet<Coordinate> dynGrid, int x, int y){
		...
		else if	(col.Equals(keyHole))	{	dynGrid.Add( new Coordinate(x,y,DungeonTile.Hole) );	}	//Hole
		else if	(col.Equals(keyWall))	{	dynGrid.Add( new Coordinate(x,y,DungeonTile.Wall) );	}	//Wall
		else if	(col.Equals(keyFloor))	{	dynGrid.Add( new Coordinate(x,y,DungeonTile.Floor) );	}	//Floor
		else if	(col.Equals(keySpawn))	{	dynGrid.Add( new Coordinate(x,y,DungeonTile.Spawner) );}	//Floor, but with Spawner
	...
	private	DungeonTile CheckDoorDirection(ref Color32[,] pixels, int x, int y){
		
		//search for side that has void, Doordirections: DoorNorth means leaving room while by going North
		if 		(x+1 <  texWidth	&& pixels[x+1,y+0].Equals(keyVoid))	{	return DungeonTile.DoorEast;	}//check right
		else if	(y+1 <  texHeight	&& pixels[x+0,y+1].Equals(keyVoid))	{	return DungeonTile.DoorNorth;	}//check up
		else if	(x-1 >= 0			&& pixels[x-1,y-0].Equals(keyVoid))	{	return DungeonTile.DoorWest;	}//check left
		else if	(y-1 >= 0			&& pixels[x-0,y-1].Equals(keyVoid))	{	return DungeonTile.DoorSouth;	}//check down
	...
	private	DungeonTile[,] CreateTileArray( HashSet<Coordinate> dynGrid){
	...
	//	print ("Size:"+sizeX+"/"+sizeY);
		...
		DungeonTile[,] roomTiles = new DungeonTile[sizeX,sizeY];
		for(int x=0; x < sizeX; x++){
			for(int y=0; y < sizeY; y++){
				roomTiles[x,y] = DungeonTile.None;
			}
		}
	...
	public class Coordinate/* : IEquatable<Coordinate>*/{
		public Coordinate(int x, int y, DungeonTile tileType){	this.x=x;this.y=y;this.tileType=tileType;	}
		public int x; //{ get; }
		public int y; //{ get; }
	...
	public Dungeon GenerateHauberkDungeon(	int?	width					= null,
		...
		tileArray = new DisjointSet(_dungeonWidth * _dungeonHeight);
		allTiles = new TileStructure[_dungeonWidth * _dungeonHeight];
		//floorManager = FloorManager.instance;

		for (var x = 0; x < _dungeonWidth; x++)
		{
			SetTile(new IntVector2(x, 0), DungeonTile.Wall);
		}
		for (var x = 0; x < _dungeonWidth; x++)
		{
			SetTile(new IntVector2(0, x), DungeonTile.Wall);
		}

		//print("size:"+_dungeonHeight+"|"+_dungeonWidth+"\n");
		for (var x = 1; x < _dungeonWidth; x++) {			
			for (var y = 1; y < _dungeonHeight; y++) {
				SetTile(new IntVector2(x, y), DungeonTile.Wall);
				allTiles[((y - 1) * _dungeonWidth) + x] = 
					new TileStructure(x,y) { tileType = _dungeon[x, y] };
		...
		for (var x = 1; x < _dungeonWidth; x += 2) {
			for (var y = 1; y < _dungeonHeight; y += 2) {		
				...
				if (GetTile(pos) != DungeonTile.Wall) continue;	//ignore already carved spaces
				_growMaze(pos);
			}
		}	
	...
	private	void _addRooms() {
		...
		for (var i = 0; i < numRoomTries; i++) {
			...
			int size			= Random.Range(1, roomExtraSize)	*2 + 1;	//rng.range(1, 3 + roomExtraSize) * 2 + 1;
			...
			FloorManager.instance.AddRoom(room);
			...
			allTiles[x + y].isPartOfRoom = true;
			...
			for(int ix = x; ix < x+width; ix++){
				...
				{
					_carve(new IntVector2(ix, iy), DungeonTile.Floor);
					//if (ix == x || ix == x + width)
					//{
					//	tileArray.Parent[((iy - 1) * _dungeonWidth) + ix] = x + y;
					//	_carve(new IntVector2(ix, iy), DungeonTile.Wall);
					//	allTiles[((iy - 1) * _dungeonWidth) + ix].tileType = _dungeon[x, y];
					//}
					//else if (iy == y || iy == y + height)
					//{
					//	tileArray.Parent[((iy - 1) * _dungeonWidth) + ix] = x + y;
					//	_carve(new IntVector2(ix, iy), DungeonTile.Wall);
					//	allTiles[((iy - 1) * _dungeonWidth) + ix].tileType = _dungeon[x, y];
					...
					//{
					//	tileArray.Parent[((iy - 1) * _dungeonWidth) + ix] = x + y;
					//	_carve(new IntVector2(ix, iy), DungeonTile.RoomFloor);
					//	allTiles[((iy - 1) * _dungeonWidth) + ix].tileType = _dungeon[x, y];
					//}
					tileArray.Parent[((iy - 1) * _dungeonWidth) + ix] = x + y;
					tilemap.SetTile(new Vector3Int(ix, iy, 0), roomFloor);
					

					allTiles[((iy - 1) * _dungeonWidth) + ix].isPartOfRoom = true;
				}	
			}
		}
	}
	...
	private	void _connectRegions() {
		...
		for(int ix = 1; ix < _dungeonWidth-1; ix++){
			...
			if (GetTile(pos) != DungeonTile.Wall) continue;
		...
		if(tryRoomsFirst){
			...
			connectors.OrderBy(delegate(IntVector2 con) {			
				...
				foreach (IntVector2 dir in cardinal) {
					...
					if (_dungeon[indexer.x, indexer.y] == DungeonTile.RoomFloor) connectedRooms++;
				}				
		...
		while (openRegions.Count > 1) {
			...
			//	var regionss = connectorRegions[pos].Select((region) => merged[region]).ToHashSet();	
				...
				if (Random.Range(0,100) < extraConnectorChance ){
					...
					foreach (IntVector2 dir in cardinal) {
						IntVector2 indexer = (pos + dir);
						if (_dungeon[indexer.x, indexer.y] == DungeonTile.Door) return true;
					}
	...
	private	void _addJunction(IntVector2 pos) {
		...
		allTiles[((pos.y - 1) * _dungeonWidth) + pos.x].isPartOfRoom = true; 
		SetTile(pos, DungeonTile.Door);
	}
		...
		while (!done) {
			...
			for(int ix = 1; ix < _dungeonWidth-1; ix++){
				for(int iy = 1; iy < _dungeonHeight-1; iy++){
					...
					var exits = 0;
					foreach(IntVector2 dir in cardinal) {
					  if (GetTile(pos + dir) != DungeonTile.Wall) exits++;
					}
					
					if (exits != 1) continue;
					
					done = false;
					SetTile(pos, DungeonTile.Wall);
	...
	private	void _streamLineCorridors(){
		...
		*/				
		...
		for(int ix = 1; ix < _dungeonWidth-1; ix++){
			for(int iy = 1; iy < _dungeonHeight-1; iy++){
				if(_dungeon[ix,iy] == DungeonTile.Floor){
		...
		foreach(List<IntVector2> trace in traces){
			...
			for(int i = 0; i< trace.Count; i++){
				...
				foreach(IntVector2 dir in cardinal) {					
					if(GetTile( trace[i] +dir ) == DungeonTile.Wall){			//if we see a wall in test direction
	...
	private	int buildLineSegment(IntVector2 current, ref List<IntVector2> source, ref List<IntVector2> target, int currentDepth, bool addAtEnd){
		...
		int exits = 0;
		foreach(IntVector2 dir in cardinal) {
			if (GetTile(current + dir) != DungeonTile.Wall){	//if there is anything other than a wall we have an exit or else, doorsteps will have at least 2 non walls (door + path)
				exits++;
			}
		}
		if(exits > 2){			
	...
	private void MultiplyDungeon(int factor){
		...
		DungeonTile[,] newDungeon	= new DungeonTile[ xSize *factor, ySize *factor ];
	...
	private	bool _canCarve(IntVector2 pos, IntVector2 direction) {
		...
		return GetTile(pos + direction * 2) == DungeonTile.Wall;
	}
	...
	private	void _carve(IntVector2 pos, DungeonTile? type = null) {
		
		SetTile(pos, type ?? DungeonTile.Floor);	// if non is stated, default is floor
		...
		for(int x = 0; x < _dungeonWidth; x++){
			for(int y = 0; y < _dungeonHeight; y++){				
				...
				if( noise < this.threshold ){
					_dungeon[x,y] = DungeonTile.Floor;				
				}else{
					_dungeon[x,y] = DungeonTile.Wall;	
				}								
			}			
		}
	...
	public Dungeon GenerateCaveDungeon(int?	width				= null,
		...
		for(int x = 0; x < _dungeonWidth; x++){				
			for(int y = 0; y < _dungeonHeight; y++){				
				_dungeon[x,y] = Map[x,y] == 1? DungeonTile.Floor:DungeonTile.Wall;
...
//Marrt: i removed the class distinction to make those parameters visibe in the Editor
	...
	public void UpdateMap(ref DungeonTile[,] grid){
		int xSize = grid.GetLength(0);
		int ySize = grid.GetLength(1);
		...
		for(int x = 0; x < xSize; x++){		
			for(int y = 0; y < ySize; y++){
				switch(grid[x,y]){
					...
					case DungeonTile.Floor:		mapTexture.SetPixel(x, y, Color.grey);		break;
					case DungeonTile.RoomFloor:	mapTexture.SetPixel(x, y, Color.white);		break;
					case DungeonTile.Wall:			mapTexture.SetPixel(x, y, Color.black);		break;
					case DungeonTile.Door:			mapTexture.SetPixel(x, y, Color.magenta);	break;
					
					//DEBUG for room Template Dungeon
					case DungeonTile.DoorNorth:	mapTexture.SetPixel(x, y, Color.yellow);	break;
					case DungeonTile.DoorEast:		mapTexture.SetPixel(x, y, Color.blue);		break;
					case DungeonTile.DoorSouth:	mapTexture.SetPixel(x, y, Color.green);		break;
					case DungeonTile.DoorWest:		mapTexture.SetPixel(x, y, Color.cyan);		break;
				}
			}
		}
